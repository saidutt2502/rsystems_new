$(document).ready(function () {
    ...
    $('#item-nav-menu_tms').css("display","block");
    $('#item-nav-menu_tms').removeClass('nav-hide');
    $('#item-nav-menu_tms').addClass('nav-show');
    ...
    $('#add_item').click(function(){
        ...
        if($('#code').val()!='' && $('#name').val()!='' && $('#threshold').val()!='')
        {
        $('#add_item').off('click');
        $.ajax({
            type: 'post',
    ...
    $('#dynamic-table').on('click','.edit-btn',function(){
        ...
        $(this).parent().parent().parent().parent().children('.input-edit').each(function(){
            ...
            $('.done-btn').click(function(){
                if($('#code_edit').val()!='' && $('#name_edit').val()!='' && $('#location_edit').val()!='' && $('#threshold_edit').val()!='')
                {
                $.ajax({
                    type: 'post',
                    url: $('#url_ajax').val(),
                    ...
                    success: function (data) {
                        if (data) {
                            ...
                            $('.selected').html('<td data-type="code_edit" class="input-edit">'+$('#code_edit').val()+'</td><td data-type="name_edit" class="input-edit">'+$('#name_edit').val()+'</td><td data-type="available_edit" class="input-edit">'+$('#available_edit').val()+'</td><td data-type="threshold_edit" class="input-edit hidden-480">'+$('#threshold_edit').val()+'</td><td data-type="location_edit" class="input-edit hidden-480">'+$('#location_edit').val()+'</td><td>'+today.toLocaleDateString("en-US", options)+'</td><td class="hidden-480"><div class="hidden-sm hidden-xs action-buttons"><a class="info" href="javascript:void(0)"><i class="ace-icon fa fa-pencil bigger-130 edit-btn"></i></a><a class="green done-btn" href="javascript:void(0)" style="display:none"><i class="ace-icon fa fa-check bigger-130"></i></a><a class="red" href="javascript:void(0)"><i class="ace-icon fa fa-trash-o bigger-130 remove-btn"></i></a></div></td>');
